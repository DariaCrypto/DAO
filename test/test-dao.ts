...
import {
  ...
  TesCContract,
  TesCContract__factory,
} from "../typechain-types";

describe("DAO", function () {
  let DAO: DAO;
  let token: Token;
  let CContract: CallContract;
  ...
  beforeEach(async function () {
    ...
    CContract = await new CallContract__factory(signers[0]).deploy();
  ...
  describe("withdraw", () => {
    ...
    it("withdrawTokens: reverted 'last voting isn't ended'", async () => {
      await DAO.deposit(defaultAmount);
      await DAO.addProposal(CContract.address, txData, "Some description");
  ...
  describe("addProposal", () => {
    it("addProposal", async () => {
      await DAO.addProposal(CContract.address, txData, "Description");
      ...
      expect(result[0]).to.eq(CContract.address);
      expect(result[1]).to.eq(txData);
      expect(result[2]).to.eq("Description");
    ...
    it("addProposal: to emit ProposalAdded", async () => {
      await expect(
        DAO.addProposal(CContract.address, txData, "Some description")
      )
  ...
  describe("finishProposal", () => {
    ...
    it("finishProposal: minimum votes less then needed ", async () => {
      ...
      await expect(DAO.finishProposal(0))
        ...
        .withArgs(0, false, CContract.address, defaultAmount, 1);
    });
    ...
    it("finishProposal: call target contract", async () => {
      ...
      await ethers.provider.send("evm_increaseTime", [period]);
      await DAO.finishProposal(0);
      expect(await CContract.current()).to.eq(5);
    });
